...
def load_spmv(filename):
    with open(f"{data_dir}/{filename}") as f:
        return json.load(f)["spmv"]
...
fp16_csr = load_spmv("with_fp16_spmv_csr_cuda_results.json")
bf16_csr = load_spmv("with_bf16_spmv_csr_cuda_results.json")
fp16_ell = load_spmv("with_fp16_spmv_ell_cuda_results.json")
bf16_ell = load_spmv("with_bf16_spmv_ell_cuda_results.json")
...
fig.tight_layout()
fig.savefig("spmv_time.pdf", bbox_inches="tight")
print("Saved spmv_time.pdf")
...
ax2.set_ylim(0.8, max(speedup_fp16 + speedup_bf16) * 1.15)
ax2.yaxis.grid(True, linestyle="--", alpha=0.7, linewidth=0.5)
ax2.set_axisbelow(True)

fig2.tight_layout()
fig2.savefig("spmv_speedup.pdf", bbox_inches="tight")
print("Saved spmv_speedup.pdf")
